...
const { allSettings, mobxStore, setAllSettings, setAllSettingsRaw, isActive, isActiveV } = require(
...
let initDQLSettingsIfNeeded = function () {
  if (!mobxStore.balanceInfo.isDefined) {
    createDqlParams(allSettings.SETTINGS_URL)

    createDqlKillPos(allSettings.SETTINGS_URL)

    createDqlLevel(allSettings.SETTINGS_URL)
    mobxStore.balanceInfo.isDefined = true
  }
}
...
function createTradingModeDropdown (SETTINGS_URL) {
  ...
  select.change(
    () => Http.httpAsyncPost(SETTINGS_URL, JSON.stringify({ tradingModeState: { tradingMode: select.val() } }),
      json => {
...
function createPlacingTypeWithBtmChangeOnSo (
  mainContainer, SETTINGS_URL, requestCreator, valExtractor, lb, fieldName, isMain) {
  const select = $('<select>')
  select.append($('<option>').val('TAKER').text('TAKER'))
...
function createComPtsSum ($feeCont) {
  ...
  mobx.autorun(r => {
    ...
    const fractionDigits =
      (allSettings.contractMode.left.startsWith('XRP') || allSettings.contractMode.right.startsWith('XRP'))
        ? 4 : 3
    const left_taker_com_pts = Number(
      (allSettings.feeSettings.leftTakerComRate / 100 * mobxStore.left_best_sam).toFixed(fractionDigits))
    const left_maker_com_pts = Number(
      (allSettings.feeSettings.leftMakerComRate / 100 * mobxStore.left_best_sam).toFixed(fractionDigits))
    const right_taker_com_pts = Number(
      (allSettings.feeSettings.rightTakerComRate / 100 * mobxStore.right_best_sam).toFixed(fractionDigits))
    const right_maker_com_pts = Number(
      (allSettings.feeSettings.rightMakerComRate / 100 * mobxStore.right_best_sam).toFixed(fractionDigits))
...
function createDqlParams (SETTINGS_URL) {
  ...
  $('<span>').
  text(`(new signals only if R_${dSym} >= R_${dSym}_open_min)`).
  addClass('spanAsHint').
  ...
  $('<span>').
  text(`(#R_preliq correction when R_${dSym} <= R_${dSym}_close_min)`).
  addClass('spanAsHint').
...
function createDqlLevel (SETTINGS_URL) {
  ...
  let label = $('<span>').html(`${dSym}_level: `).appendTo($cont)
  label.prop('title', 'Preliq выполняется при уловиях:\n'
    + `L_${dSym} <= L_${dSym}_close_min && L_${dSym} >= ${dSym}_level, или\n`
    + `R_${dSym} <= R_${dSym}_close_min && R_${dSym} >= ${dSym}_level.`)
